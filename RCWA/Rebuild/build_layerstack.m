...
for i=1:numel(layer)
    ...
    if layer(i).input==0
        ...
        if isa(layer(i).input,'Surface')
            ...
            %[Z, L, Lrecalc] = realsurf(input,layer(i).roughdim, device.optimRough, device.reverse, device.tolerance);
        end
