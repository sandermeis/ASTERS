...
res = param.res;
size = param.size;
height = size*tand(param.angle);

surf_wedge = Surface(res, size);
surf_wedge.addFeature(Feature(res, size, height, "WedgeX"),1,1)
surf_wedge.placeFeatures("PBC", true, "mode", "add")
%surf_wedge.plot

surfaces{1} = surf_wedge;