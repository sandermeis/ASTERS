...
%% Checking and modifying created params
for f = 1:numel(param)

    if round(param(f).nharm) == param(f).nharm && param(f).nharm > 0
        param(f).Hmax = 2 * param(f).nharm + 1;
    else
        error("Number of harmonics (nharm) must be a positive integer.")
    end

    % Change wavelengths where diffraction orders are at cut-off
    cutoffs = find_cutoff(param(f).size, param(f).wavelengthArray, param(f).Hmax, param(f).theta);
    if ~isempty(cutoffs)
        warning("Wavelengths %s at cutoff, offsetting by 1E-6 to prevent NaN values.",strjoin(compose("%d",param(f).wavelengthArray(cutoffs)),", "))
        for co = 1:numel(cutoffs)
            param(f).wavelengthArray(cutoffs(co)) = param(f).wavelengthArray(cutoffs(co)) - 0.000001;
        end
    end
    
    if round(param(f).res) == param(f).res && param(f).res > 0
        % Require res > 2 x Hmax, due to how convolution matrices work
        if param(f).res < 2*param(f).Hmax
            error("Res must be at least 2x Hmax (4 * nharm + 2).")
        end
    else
        error("Matrix size (res) must be a positive integer.")
    end

    % Restrict fields to 1 wavelength due to .mat file size constraint
    if param(f).calcFields && (numel(param(f).wavelengthArray) > 1)
        error("Due to file size constraints it is only possible to calculate fields for 1 wavelength.")
    end
...
eps_lab_ref   = import_permittivities({C_ref(:, 1)});
...
eps_lab_trn   = import_permittivities({C_trn(:, 1)});
...
function unique_cutoffs = find_cutoff(size, wavelengthArray, nHarm, angle)

harms = -(nHarm - 1) / 2 : (nHarm - 1) / 2;

sols = zeros(numel(wavelengthArray), numel(harms));
for i = 1:numel(wavelengthArray)
    for j = 1:numel(harms)
        sols(i, j) = sin(angle) - harms(j) * wavelengthArray(i) / size;
    end
end

[p, ~] = find((sols == 1) | (sols == -1));

unique_cutoffs = unique(p);
end
